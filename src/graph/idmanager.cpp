...
namespace graph {
   ...
   * --------------------------------------------------------------------------------------*/
  void IdScanner::Reclaim(type::gid id) {
    if(!this->m_active) {
   ...
   * --------------------------------------------------------------------------------------*/
  void IdManager::Reclaim(type::gid id, Storeable::Concept concept){
    if(!this->m_isopen) {
      std::cout << "[ID MANAGER] Error - trying to reclaim an id while closed." << std::endl;
      return;
    }
    IdScanner *item = this->m_cache[concept];
    if(item == 0x0) {
...
//  }
   ...
   * --------------------------------------------------------------------------------------*/
  type::gid IdManager::NextGraphId(Storeable::Concept concept){
    if (!this->m_isopen) {
  ...
  bool IdManager::Load() {
    ...
    for(std::uint16_t i = 0; i < typecnt; i++) {
      ...
      if (item != 0x0) {
        ...
        item->SetCounter((type::gid)counter);
      } else {
    ...
    for (const auto &pair : this->m_cache) {
      ...
      for(std::uint32_t i = 0; i < reclaimCount; i++) {
        ...
        pair.second->Reclaim((type::gid)id);
      }
    }
