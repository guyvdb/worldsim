...
namespace graph {
  namespace store {
    ...
    Store::Store(std::string filename, std::size_t pagesize, std::size_t recordsize, Storeable::Concept concept) :
      ...
      m_isopen(false), m_lastError(graph::type::ErrorNone),m_concept(concept), m_file(0x0)/*,m_accumulator(0x0)*/ {
...
//    }
     ...
     * --------------------------------------------------------------------------------------*/
    bool Store::ReadPage(int page, graph::type::ByteBuffer *buffer) {
     ...
     *
     * --------------------------------------------------------------------------------------*/
    bool Store::WritePage(int page, graph::type::ByteBuffer *buffer) {
      long offset = (long)page * (long)this->m_pagesize;
    ...
    bool Store::Scan(Scanner *scanner) {
      ...
      graph::type::ByteBuffer buffer(this->m_recordsize);
