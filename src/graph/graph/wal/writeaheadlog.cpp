...
namespace graph {
  ...
  namespace tx {
    ...
    TransactionLog::TransactionLog(const char *logdir) : m_logdir(logdir),
      m_file(0x0), m_isopen(false), m_lastError(graph::type::ErrorNone) {
    ...
    bool TransactionLog::Open() {
      ...
      if(this->m_isopen) {
        this->m_lastError = graph::type::ErrorFileAlreadyOpen;
        return false;
      }
     ...
     *
     * --------------------------------------------------------------------------------------*/
    bool TransactionLog::FrameAppend(type::gid id, Storeable::Concept concept, TransactionOp op, graph::type::ByteBuffer *buffer){
