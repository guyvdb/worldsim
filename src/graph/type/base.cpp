...
namespace graph {
  namespace type {
    ...
    std::string FixedString::GetValue() {
      return this->m_value;
    }
    ...
    void FixedString::WriteValue(ByteBuffer *buffer, int offset) {
      int len = this->m_value.size();

      std::cout << "m_value=" << this->m_value << ", size=" << this->m_value.size() << std::endl;


      std::cout << "Write FixedString len=" << len << ", offset=" << offset << ", value=" << this->m_value << ", encoded=";
