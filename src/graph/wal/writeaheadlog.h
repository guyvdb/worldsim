...
namespace graph {
  ...
  namespace tx {
    ...
    class FrameRecord {
      public:
        FrameRecord(type::gid id, Storeable::Concept concept, TransactionOp op, graph::type::ByteBuffer *buffer) : m_id(id), m_concept(concept), m_op(op), m_buffer(buffer) {}

        type::gid Id() { return this->m_id; }
        Storeable::Concept Type() { return  this->m_concept; }
        TransactionOp Op() { return this->m_op; }
        graph::type::ByteBuffer *Buf() { return this->m_buffer; }
    ...
    class TransactionLog {
      public:
        ...
        void Lock();
        void Release();
        type::txid FrameStart();
        bool FrameAppend(type::gid id, Storeable::Concept type, TransactionOp op, graph::type::ByteBuffer *buffer);
        bool FrameEnd();
      private:
        bool LastTransactionNumber(type::txid *result);
